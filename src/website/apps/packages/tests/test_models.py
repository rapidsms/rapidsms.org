...
from website.apps.tests.base import WebsiteTestBase
...
class PackageModelTest(WebsiteTestBase):
    ...
    def test_update_from_pypi_200_release_date(self):
        package = PackageFactory.create()
        upload_time = datetime.datetime.strftime(datetime.datetime.now(), PYPI_DATE_FORMAT)
        pypi_request = MockPyPIRequest(status_code=200, upload_time=upload_time)
        updated = package.update_from_pypi(pypi_request)
        self.assertTrue(updated)
        release_date = datetime.datetime.strftime(package.release_date, PYPI_DATE_FORMAT)
